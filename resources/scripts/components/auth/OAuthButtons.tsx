...
function OAuthButton({ href, provider, label }: OAuthButtonProps) {
    const { bg, border, textColor, hoverBg, font } = providerStyles[provider];
...
export default function OAuthButtons({ google, discord, github }: OAuthButtonsProps) {
    ...
    const enabled = [google && 'google', discord && 'discord', github && 'github'].filter(Boolean) as Array<
        'google' | 'discord' | 'github'
    >;

    if (enabled.length === 0) return null;

    const labels: Record<'google' | 'discord' | 'github', string> = {
        google: 'Sign in with Google', // required per Google branding guidelines
        discord: 'Continue with Discord',
        github: 'Continue with GitHub',
    };

    const hrefs: Record<'google' | 'discord' | 'github', string> = {
        ...
        <div>
            ...
            >
                {enabled.map((provider) => (
                    <OAuthButton key={provider} href={hrefs[provider]} provider={provider} label={labels[provider]} />
                ))}
            </div>
        </div>
    );
